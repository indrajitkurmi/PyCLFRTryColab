...
void LoadDemModel()
{
    // load the DEM 
    // -----------------------
    dem.Load("../data/T20200207F2/dem.obj");
    demTexture = loadImageSTBI("../data/T20200207F2/dem.png", true); // tell stb_image.h to flip loaded texture's on the y-axis
}
...
void RendererInit()
{
    ...
    lf = generator.Generate("../data/T20200207F2/thermal_GPS_Corr.json", "../data/T20200207F2/thermal_ldr512/");
...
void Completerender()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //lightProjection = glm::perspective(glm::radians(45.0f), (GLfloat)SHADOW_WIDTH / (GLfloat)SHADOW_HEIGHT, near_plane, far_plane); // note that if you use a perspective projection matrix you'll have to change the light position as the current light position isn't enough to reflect the whole scene
	    //imgProjection = glm::perspective(glm::radians(50.815436217896945f), 1.0f, near_plane, far_plane);
	    imgProjection =SetProjectionMatrix();	
